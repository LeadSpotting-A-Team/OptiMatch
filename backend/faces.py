...
def is_valie_face(face_object : dict , confidence_threshold : float = 0.9):
    ...
    if 'facial_area' in face_object and (face_object['facial_area']['w'] < 64 or face_object['facial_area']['h'] < 64):
        return False
    if 'box' in face_object and (face_object['box'][2] < 64 or face_object['box'][3] < 64):
        return False
...
def save_faces_to_file(faces_coordinates : list[Face_Coordinates] , image , path : str , post_metadata : metadata_module.Post_Metadata):
    ...
    for face in faces_coordinates:
        ...
        face_id = str(int(time.time())) + "_" + str(i) #generate a unique face id using the current timestamp and the index of the face
